...
class MPIAdamOptimizer(tf.train.AdamOptimizer):
    ...
    def calc_grads(self, loss, variable_list, **kwargs):
        ...
        avg_flattened_grad = tf.py_func(_collect_grads, [flat_gradients], tf.float32)
...
def mpi_statistics_scalar(x, with_min_and_max=False):
    ...
    average = global_sum / global_n

    global_sum_of_sq = mpi_sum(np.sum(x - average)**2)
    std = np.sqrt(global_sum / global_n)
