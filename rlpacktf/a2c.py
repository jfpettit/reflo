...
class A2C:
    ...
    def init_common(
        ...
        actor,
        critic,
        actor_critic_hidden_sizes,
        epoch_interactions,
        gamma,
        policy_learning_rate,
        valuef_learning_rate,
        valuef_train_iters,
        lam,
        max_episode_length,
        track_run,
        ...
        if track_run:
            assert (
                track_dir is not None
            ), "In order to track the run you must provide a directory to save run details to."
        ...
        advantage_ph, return_ph, logprobs_old_ph = utils.placeholders(None, None, None)

        value_f = critic(
            actor_critic_hidden_sizes + [1],
        ...
        if isinstance(env.action_space, Discrete):
            self.policy = actor(
                actor_critic_hidden_sizes + [env.action_space.n],
                activation=tf.nn.tanh,
                activate_final=False,
                name="policy",
            )
            ...
            logprobs = tf.reduce_sum(
                tf.one_hot(action_ph, depth=env.action_space.n) * logprobs_all, axis=1
            )
            logprobs_action = tf.reduce_sum(
                tf.one_hot(pi, depth=env.action_space.n) * logprobs_all, axis=1
            )

        elif isinstance(env.action_space, Box):
            self.policy = actor(
                actor_critic_hidden_sizes + [env.action_space.shape[0]],
                activation=tf.nn.tanh,
                activate_final=False,
                name="policy",
            )
            ...
            log_std = tf.convert_to_tensor(
                -0.5 * np.ones(act_dim, dtype=np.float32), dtype=tf.float32
            )
    ...
    def init_loss_funcs(self, **kwargs):
        self.policy_loss = -tf.reduce_mean(kwargs["logprobs"] * kwargs["advantage_ph"])
    ...
    def learn(
        ...
        if save_policies:
            assert (
                self.track_dir is not None
            ), "Must provide directory to log policies to."
        ...
        for epoch in range(epochs):
            ...
            for step in range(self.local_epoch_interactions):
                ...
                self.experience_buffer.push(obs, action, reward, value, logprobs_step)
                ...
                if over or (step == self.local_epoch_interactions - 1):
                    ...
                    last_value = (
                        ...
                        if done
                            ...
                            self.state_values,
                            feed_dict={self.observation_ph: obs.reshape(1, -1)},
                        )
                    )
                    self.experience_buffer.end_trajectory(last_value=last_value)
                    if over:
                        epochlen.append(ep_length)
                        epochrew.append(ep_return)
                        all_train_ep_rews.append(ep_return)
                        self.logger.store(EpReturn=ep_return, AvgEpLength=ep_length)
                    obs, reward, done, ep_return, ep_length = (
                        self.env.reset(),
                        0,
                        False,
                        0,
            ...
            if self.track_run:
                self.tracker.update_metrics(
                    mean_episode_return=np.mean(epochrew),
                    mean_episode_length=np.mean(epochlen),
            ...
            if save_policies and np.mean(epochrew) > last_save_ret:
                ...
                saver.save(
                    self.sess,
                    self.track_dir
                    + "/model_checkpoints/model_checkpoint_"
            ...
            if render_epochs is not None and epoch in render_epochs:
                self.watch_model(
                    render_frames=render_frames, save_renders=save_renders, epoch=epoch
                )
    ...
    def watch_model(self, render_frames=250, save_renders=False, epoch=None):
        if save_renders:
            ...
            local_env = wrappers.Monitor(
                self.env, path, video_callable=lambda episode_id: True, force=True
            )
        else:
            local_env = self.env
        ...
        for _ in range(render_frames):
            action, _, _ = self.sess.run(
                self.get_action, feed_dict={self.observation_ph: obs.reshape(1, -1)}
            )
            obs, _, done, _ = self.env.step(action[0])
            local_env.render()
            if done:
                obs = local_env.reset()
        local_env.close()
